...
static int tas2562_set_samplerate(struct tas2562_data *tas2562, int samplerate)
{
	...
	snd_soc_update_bits(tas2562->codec, TAS2562_TDM_CFG0,
		...
		int slots, int slot_width)
{
	...
	ret = snd_soc_write(codec, TAS2562_TDM_CFG3, slots_cfg);
	if (ret < 0)
		return ret;

	switch (slot_width) {
	case 16:
		ret = snd_soc_update_bits(codec, TAS2562_TDM_CFG2,
	...
	ret = snd_soc_update_bits(codec, TAS2562_TDM_CFG5,
				  TAS2562_TDM_CFG5_VSNS_SLOT_MASK,
				  tas2562->v_sense_slot);
	if (ret < 0)
		return ret;

	ret = snd_soc_update_bits(codec, TAS2562_TDM_CFG6,
...
static int tas2562_set_bitwidth(struct tas2562_data *tas2562, int bitwidth)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		snd_soc_update_bits(tas2562->codec, TAS2562_TDM_CFG2,
	...
	val = snd_soc_read(tas2562->codec, TAS2562_PWR_CTRL);
	if (val < 0)
		return val;
	...
	ret = snd_soc_update_bits(tas2562->codec, TAS2562_TDM_CFG6,
...
static int tas2562_set_dai_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	ret = snd_soc_update_bits(codec, TAS2562_TDM_CFG1,
...
static int tas2562_mute(struct snd_soc_dai *dai, int mute)
{
	...
	return snd_soc_update_bits(codec, TAS2562_PWR_CTRL,
				   TAS2562_MODE_MASK,
				   mute ? TAS2562_MUTE : 0);
}
...
static int tas2562_codec_probe(struct snd_soc_codec *codec)
{
	...
	ret = snd_soc_update_bits(codec, TAS2562_PWR_CTRL, TAS2562_MODE_MASK,
				  TAS2562_MUTE);
	if (ret < 0)
		return ret;
...
#ifdef CONFIG_PM
static int tas2562_suspend(struct snd_soc_codec *codec)
{
	struct tas2562_data *tas2562 = snd_soc_codec_get_drvdata(codec);
...
static int tas2562_resume(struct snd_soc_codec *codec)
{
	struct tas2562_data *tas2562 = snd_soc_codec_get_drvdata(codec);
			     ...
			     struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		ret = snd_soc_update_bits(codec, TAS2562_PWR_CTRL,
					  TAS2562_MODE_MASK,
					  TAS2562_MUTE);
		if (ret)
			goto end;
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = snd_soc_write(codec, TAS2562_DVC_CFG4, (reg_val & 0xff));
	if (ret)
		return ret;
	ret = snd_soc_write(codec, TAS2562_DVC_CFG3, ((reg_val >> 8) & 0xff));
	if (ret)
		return ret;
	ret = snd_soc_write(codec, TAS2562_DVC_CFG2, ((reg_val >> 16) & 0xff));
	if (ret)
		return ret;
	ret = snd_soc_write(codec, TAS2562_DVC_CFG1, ((reg_val >> 24) & 0xff));
	if (ret)
		return ret;
...
static const struct snd_soc_codec_driver soc_codec_dev_tas2110 = {
...
static const struct snd_soc_codec_driver soc_codec_dev_tas2562 = {
	.probe			= tas2562_codec_probe,
	.suspend		= tas2562_suspend,
	.resume			= tas2562_resume,
	.set_bias_level		= tas2562_set_bias_level,
			 ...
			 const struct i2c_device_id *id)
{
	...
	if (data->model_id == TAS2110)
		return snd_soc_register_codec(dev, &soc_codec_dev_tas2110,
					      tas2562_dai,
					      ARRAY_SIZE(tas2562_dai));

	return snd_soc_register_codec(dev, &soc_codec_dev_tas2562, tas2562_dai,
				      ARRAY_SIZE(tas2562_dai));

}

static int tas2562_remove(struct i2c_client *client)
{
